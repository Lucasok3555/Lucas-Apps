        ...
        while True:
            ...
            if contador % 3 == 0:
                try:
                    ...
                    interfaces = stats.get("interfaces", []) if isinstance(stats, dict) else []
                    print(f"[{time.strftime('%H:%M:%S')}] Nó ativo | Interfaces: {len(interfaces)}")
                    ...
                    print(f"[{time.strftime('%H:%M:%S')}] Nó ativo | (stats indisponíveis: {e})")
